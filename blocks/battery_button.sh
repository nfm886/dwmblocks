...
read -r rate </sys/class/power_supply/BAT0/capacity
[ "$rate" = 0 ] && notify-send "Battery fully charged" && exit

read -r ac </sys/class/power_supply/ADP0/online
read -r charge_now </sys/class/power_supply/BAT0/energy_now

if [ "$ac" = 1 ] ; then
    read -r charge_full </sys/class/power_supply/BAT0/energy_full
